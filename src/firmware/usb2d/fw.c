...
#define POT_ADD_WR 0x54
#define POT_CMD_WR 0x11
#define SHAD0 0x8009
#define SHAD1 0x800A
//-----------------------------------------------------------------------------
// Random Macros
...
extern void shadow0(void);
extern void shadow1(void);

extern BYTE	potdata;
extern BYTE ndiv;
extern BYTE shad_data;
extern bit tasstop;
...
//   WORD  shadowr;
   ...
   Sleep = FALSE;
/*
   while(EP1OUTCS & bmEPBUSY);		// Is there something in the OUT1BUF buffer?
...
//      setdebug();
	    ...
	    if(!(EP1OUTCS & bmEPBUSY))		// Is there something in the OUT1BUF buffer?
      {
        count = EP1OUTBC;		
        if(count == 3) {
          shadow0();
          EP1INBUF[0] = shad_data;
//          shadowr = (int)shad_data;
          shadow1();
          EP1INBUF[1] = shad_data;
          EP1INBC = 0x03;
//          shadowr = (shadowr << 8) | (int)(shad_data & 0x00FF);
          if ((potdata != EP1OUTBUF[0]) || (ndiv != EP1OUTBUF[1])) {
      	    potdata = EP1OUTBUF[0];
            ndiv = EP1OUTBUF[1];
            EP1INBUF[2] = ndiv;
//            setpot();
