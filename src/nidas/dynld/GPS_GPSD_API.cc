...
const int GPS_GPSD_API::GGA_SAMPLE_ID = 1;

const int GPS_GPSD_API::RMC_SAMPLE_ID = 2;

const int GPS_GPSD_API::HDT_SAMPLE_ID = 3;

NIDAS_CREATOR_FUNCTION(GPS_GPSD_API)

GPS_GPSD_API::GPS_GPSD_API():SerialSensor(),
...
dsm_time_t GPS_GPSD_API::parseRMC(const char* input,double *dout,int nvars,
  dsm_time_t tt) throw()
{
    char sep = ',';
...
dsm_time_t GPS_GPSD_API::parseGGA(const char* input,double *dout,int nvars,
  dsm_time_t tt) throw()
{
    char sep = ',';
...
dsm_time_t GPS_GPSD_API::parseHDT(const char* input,double *dout,int,
  dsm_time_t tt) throw()
{
    double val;
...
bool GPS_GPSD_API::checksumOK(const char* rec,int len)
{
...
bool GPS_GPSD_API::process(const Sample* samp,list<const Sample*>& results)
  throw()
{
    dsm_time_t ttfixed;
...
SampleScanner* GPS_GPSD_API::buildSampleScanner()
  throw(n_u::InvalidParameterException)
{
