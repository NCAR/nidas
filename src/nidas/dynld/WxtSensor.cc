    ...
    for (unsigned int i = 0; i < sizeof(paramSet) / sizeof(paramSet[0]); i++)
    {
        ...
        if (param->getLength() != 1)
            throw nidas::util::InvalidParameterException(
                getName(), "parameter",
                string("bad length for ") + paramSet[i].name);
        ...
        for (int i = 0; vi.hasNext(); i++)
        {
            ...
            if (uvId != 0)
            {
                if (_uvId == 0)
                    ...
                    throw nidas::util::InvalidParameterException(
                        getName() + " derived U and V must be in one sample");
            }
        }
    }
    ...
    for (; si != sscanfers.end(); ++si)
    {
        ...
        if (nfield != sscanf->getNumberOfFields())
        {
            throw nidas::util::InvalidParameterException(
                ...
                "and only 1 per comma separated field. Use %*f to skip a "
                "field");
        }
    }
...
int
WxtSensor::scanSample(AsciiSscanf* sscanf, const char* inputstr,
                      float* dataptr)
{
    const SampleTag* stag = sscanf->getSampleTag();
    ...
    while (fi != field_formats.end() && vi != variables.end())
    {
        ...
        if (fi == field_formats.begin() && fi->length() == si->length() &&
            fi->length() == 3 && (sf[0] == '0' || sf[0] == '1') &&
        ...
        if (fi->substr(0, 2) == si->substr(0, 2))
        {
            ...
            {
                value = floatNAN;
                VLOG(("field NOT scanned, set to NaN (")
                     << *fi << "): " << *si << " = " << value);
            }
            ++si;
        }
        ...
        {
            DLOG(("") << getName() << ": format field " << *fi
                      << " does not match next sample field " << *si
...
bool
WxtSensor::process(const Sample* samp,
                   std::list<const Sample*>& results) throw()
{
    ...
    for (; si != results.end(); ++si)
    {
        ...
        if (csamp->getId() != _speedDirId)
            continue;

        unsigned int slen = csamp->getDataLength();

        if ((signed)slen <= _speedIndex)
            continue;
        if ((signed)slen <= _dirIndex)
            continue;
