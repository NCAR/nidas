    ...
    throw(n_u::InvalidParameterException)
{
  ...
  // Currently the driver must have a end or start-of-message separator,
  // so this needs to be re-visited.
...
void SPP200_Serial::sendInitString() throw(n_u::IOException)
{
  ...
  read(&init_return, sizeof(Init200_blk));

  if (memcmp(&init_return, &_setup_pkt, sizeof(Init200_blk)) != 0)
    throw n_u::IOException(getName(), "S200 init return packet doesn't match", errno);
}
