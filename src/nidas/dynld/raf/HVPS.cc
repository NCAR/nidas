...
NIDAS_CREATOR_FUNCTION_NS(raf, HVPS)

// This will ve a Type32 format, not Type48
const unsigned char HVPS::_syncString[] = { 0xaa, 0xaa, 0xaa };
...
return false;  // Remove when ready.
    ...
    std::cout << "HVPS::processImageRecord, len = " << slen << " - " <<
...
/*---------------------------------------------------------------------------*/
void HVPS::printStatus(std::ostream& ostr)
{
