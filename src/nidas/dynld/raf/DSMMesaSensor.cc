...
void DSMMesaSensor::outportbwswap(unsigned thebyte)
{
  unsigned char* ptr;
...
/* Function: selectfiletype
   ...
   Returns: Nothing.
*/
void DSMMesaSensor::selectfiletype(void)
{
  unsigned char b[14] ;
...
/*---------------------------------------------------------------------------*/
static long DSMMesaSensor::filelengthq(FILE *f)
{
  long curpos, len ;
...
void DSMMesaSensor::close() throw(n_u::IOException)
{
  cerr << __PRETTY_FUNCTION__ << endl;
       ...
       child = child->getNextSibling())
  {
    ...
    if (!elname.compare("mesacfg")) {
      ...
      if ( xchild.getAttributeValue("desc").c_str()[0] == 'C' ) {
        ...
        throw n_u::InvalidParameterException (__PRETTY_FUNCTION__,
