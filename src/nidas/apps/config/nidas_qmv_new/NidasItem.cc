...
NidasItem::NidasHeaderModel NidasItem::_DSM_HeaderModel((DSMConfig*)0);
QHeaderView NidasItem::_DSM_HeaderView(Qt::Horizontal,0);

QHeaderView *NidasItem::getChildHeaderView()
{
  switch(this->nidasType){
  case DSMCONFIG:
    {
    _DSM_HeaderView.setModel(_DSM_HeaderModel.getQAIM());
    return &_DSM_HeaderView;
    }
  /* default: fall thru */
  } // end switch
return new QHeaderView(Qt::Horizontal,0);
}


NidasItem::NidasHeaderModel::NidasHeaderModel(DSMConfig *dsm, QWidget *parent) : QAbstractItemModel(parent)
{
dummyItem = new NidasItem(dsm,0,0);
}

int NidasItem::NidasHeaderModel::columnCount(const QModelIndex &parent) const
{
...
//if (parentItem !xxx dummyItem) return 0; // check if parent passed in is same type as ourself (dummyItem)

return parentItem->childColumnCount();
}

QVariant NidasItem::NidasHeaderModel::headerData(int section, Qt::Orientation orientation, int role) const
{
return dummyItem->childLabel(section);
}
