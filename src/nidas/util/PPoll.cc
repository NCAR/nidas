...
int PPoll::poll()
{
    int nfdEvents = ::ppoll(_pFds, _nfds, &_timeout, &_sigmask);

    if (nfdEvents < 0)
        throw nidas::util::IOException(""/*getName()*/,"PPoll::poll()",errno);
    if (nfdEvents == 0)
        throw nidas::util::IOTimeoutException(""/*getName()*/,"PPoll::poll()");

    return nfdEvents;
}

void PPoll::addPollee(int fd, int events)
{
...
void PPoll::removePollee(int fd)
{
...
const event_data PPoll::getNextPolleeEvents(uint64_t curIdx)
{
