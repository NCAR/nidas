 ...
 * such as from an A2D.  The A2D driver modules assign the initial
 * time tag based on the time of the read of the A2D FIFO by the
 * interrupt service routine or polling method. These time tags
 * may be late due to ISR or polling latency. This function keeps
 * a running mean of the differences between the expected and actual
 * time tags, and uses it to generate a time series with approximately
 * the requested delta-T, but which can slowly drift to account for
 * any clock drift of the A2D, or to correct an error in the initial
 * time of the series.
 *
...
dsm_sample_time_t screen_timetag(struct screen_timetag_data* td, dsm_sample_time_t tt)
{
    ...
    /* Estimated time, based on a fixed delta-T from tt0 */
    tt_est = td->tt0 + toff;

    /* estimated time tag error */
    tterr = (tt - tt_est);
    ...
    /* compute average of errors in units of usecs to
     * improve precision.
     */
    ...
    /*
     * Subtract the smoothed error from tt0, which could 
