...
DSMEngine::DSMEngine() :
        _externalControl(false), _disableAutoconfig(false), _runState(DSM_RUNNING), _command(DSM_RUN), _syslogit(
                true), _configFile(), _configSockAddr(), _project(0), _dsmConfig(
                0), _selector(0), _pipeline(0), _statusThread(0), _xmlrpcThread(
                0), _outputSet(), _outputMutex(), _logLevel(defaultLogLevel), _signalMask(), _myThreadId(
                ::pthread_self()), _app("dsm")
{
    try {
...
int DSMEngine::main(int argc, char** argv) throw ()
{
    ...
    ILOG(
            ("dsm: sample pools: #s%d,#m%d,#l%d,#o%d\n", charPool->getNSmallSamplesIn(), charPool->getNMediumSamplesIn(), charPool->getNLargeSamplesIn(), charPool->getNSamplesOut()));

    logPageFaultDiffs(minflts, majflts, nswap);

    if (engine.getCommand() == DSM_SHUTDOWN)
        n_u::Process::spawn("halt");
    else if (engine.getCommand() == DSM_REBOOT)
        n_u::Process::spawn("reboot");
...
int DSMEngine::parseRunstring(int argc, char** argv) throw ()
{
    ...
    NidasAppArg DisableAutoConfig("-n,--no-autoconfig", "", 
    ...
    _app.enableArguments(
            _app.loggingArgs() | _app.Version | _app.Help | _app.Username
                    | _app.Hostname | _app.DebugDaemon | ExternalControl | DisableAutoConfig);
    ...
    //cerr << "DSMEngine::parseRunstring(): num args = " << args.size() << endl << endl << flush;
    ...
    for (; !quitCommand(_command);) {
        ...
        try {
            if (_configFile.length() == 0) {
                projectDoc = n_c::requestXMLConfig(false, _configSockAddr,
                        &_signalMask);
            }
            else {
        ...
        if (_dsmConfig->getDerivedDataSocketAddr().getPort() != 0) {
            DerivedDataReader::createInstance(
                    _dsmConfig->getDerivedDataSocketAddr());
        }
        // start your sensors
        try {
            openSensors();
        ...
        if (_dsmConfig->getStatusSocketAddr().getPort() != 0) {
            _statusThread = new DSMEngineStat("DSMEngineStat",
                    _dsmConfig->getStatusSocketAddr());
            _statusThread->start();
        }
...
void DSMEngine::waitForSignal(int timeoutSecs)
{
    ...
    if (sig < 0) {
        ...
        if (errno == EINTR)
            PLOG(("DSMEngine::waitForSignal(): unexpected signal"));
        else
            PLOG(
                    ("DSMEngine::waitForSignal(): ") << n_u::Exception::errnoToString(errno));
        return;
    }
...
{
    if (_xmlrpcThread)
        return _xmlrpcThread->registerSensor(devname, sensor);
}
...
void DSMEngine::openSensors() throw (n_u::IOException)
{
    ...
    _pipeline->setRawLateSampleCacheSize(
            _dsmConfig->getRawLateSampleCacheSize());
    _pipeline->setProcLateSampleCacheSize(
            _dsmConfig->getProcLateSampleCacheSize());
...
void DSMEngine::connectOutputs() throw (n_u::IOException)
{
    ...
    for (oi = outputs.begin(); oi != outputs.end(); ++oi) {
        ...
        DLOG(
                ("DSMEngine requesting connection from SampleOutput '%s'.", output->getName().c_str()));
        SampleSource* src;
        if (output->isRaw())
            src = _pipeline->getRawSampleSource();
        else
            src = _pipeline->getProcessedSampleSource();
        output->addSourceSampleTags(src->getSampleTags());
        SampleOutputRequestThread::getInstance()->addConnectRequest(output,
                this, 0);
    }
}
...
{
    if (output->isRaw())
        _pipeline->getRawSampleSource()->removeSampleClient(output);
    else
        _pipeline->getProcessedSampleSource()->removeSampleClient(output);
    ...
    if (delay < 0)
        return;
    SampleOutputRequestThread::getInstance()->addConnectRequest(orig, this,
            delay);
}
...
{
    // need to always init the sensors, since now they process their own samples for
    // periodic QC.
