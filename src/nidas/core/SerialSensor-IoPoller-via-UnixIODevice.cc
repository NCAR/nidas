                               ...
                               bool backOffTimeout, int retryTimeoutFactor)
{
    ...
    if (msecTimeout >= MSECS_PER_SEC) {
        ...
        try {
            // res = poller.poll();
            numChars = _serialDevice->read((char*)buf, (unsigned long)len, msecTimeout);
        ...
        for (int i=0; res <= 0 && i < 4; ++i) {
            ...
            try {
                // res = poller.poll();
                numChars = _serialDevice->read((char*)buf, (unsigned long)len, tmpto);
            } catch (IOTimeoutException& e) {
                // DLOG(("Select timeout error on: ") << getDeviceName() << e.what());
                // return res;
            } catch (IOException& e) {
    ...
    // VLOG(("SerialSensor::readResponse(): Select successful, cleaning up the data..."));
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            for (int j = 0; j < 10; ++j) {
                if ((i*10 + j) > numCharsRead)
                    break;
                const char const* fmt = "0x%02x ";
                snprintf(&(hexBuf[j * 5]), 6, fmt, respBuf[(i * 10) + j]);
            }
