        ...
        throw (n_u::InvalidParameterException)
{
    ...
    if (node->hasAttributes()) {
        ...
        for (int i = 0; i < nSize; ++i) {
            ...
            std::string sval;
            if (getDSMSensor())
                sval = getDSMSensor()->expandString(aval);
            else
                sval = Project::getInstance()->expandString(aval);
            ...
            if (aname == "id") {
                ...
                if (ist.fail() || rate < 0.0) {
                    ...
                    throw n_u::InvalidParameterException(ost.str(), aname,
                            sval);
                }
                setRate(rate);
            }
    ...
    for (child = node->getFirstChild(); child != 0; child = child->getNextSibling()) {
        ...
        if (elname == "variable") {
            ...
            if (getDSMSensor())
                dict = &getDSMSensor()->getDictionary();
            Parameter* parameter = Parameter::createParameter(
                    (xercesc::DOMElement*) child, dict);
...
xercesc::DOMElement* SampleTag::toDOMParent(xercesc::DOMElement* parent,
        bool complete) const throw (xercesc::DOMException)
{
    xercesc::DOMElement* elem = parent->getOwnerDocument()->createElementNS(
