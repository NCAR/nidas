...
SerialSensor::SerialSensor():
    ...
    _deviceState(AUTOCONFIG_UNSUPPORTED),_configMode(NOT_ENTERED), _initPowerState(n_u::POWER_OFF),
                               ...
                               bool backOffTimeout, int retryTimeoutFactor)
{
    ...
    if (msecTimeout >= MSECS_PER_SEC) {
        try {
            numChars = read((char*)buf, (unsigned long)len, msecTimeout);
        } catch (EOFException e) {
            DLOG(("SerialSensor::readResponse(): EOFException: port/device: %s, read error: %s", 
                  getName().c_str(), e.what()));
        } catch (IOException e) {
            DLOG(("SerialSensor::readResponse(): IOException: port/device: %s, read error: %s", 
                  getName().c_str(), e.what()));
        }
    }
    else {
        ...
        int retryTimeout = msecTimeout;
        for (int i=0; numChars != 0 && i < 4; ++i) {
            VLOG(("SerialSensor::readResponse(): select loop: i: %i; retryTimeout: %i;",
                  i, retryTimeout));
            try {
                numChars = read((char*)buf, (unsigned long)len, msecTimeout);
            } catch (EOFException e) {
            ...
            VLOG(("SerialSensor::readResponse(): select loop: read result: %i; ", numChars));

            if (numChars == 0 && backOffTimeout) {
                VLOG(("SerialSensor::readResponse(): select loop backing off: i: %i; ", i));
                retryTimeout *= retryTimeoutFactor;
            }
        }
    }
    ...
    if (checkPrintable) {
        VLOG(("SerialSensor::readResponse(): Read successful, checking for non-printables..."));
                                     ...
                                     bool checkPrintable, int retryTimeoutFactor)
{
    ...
    bool gotJunk = charsReadStatus & 0x80000000;
    DLOG(("SerialSensor::readEntireResponse(): gotJunk: %s", (gotJunk ? "true" : "false")));
    ...
    int i=0;
    for (; (!gotJunk && numCharsRead && bufRemaining > 0); ++i) {
        charsReadStatus = readResponse(&cbuf[bufIdx], bufRemaining, 1,
                                       checkPrintable, true, retryTimeoutFactor);
        gotJunk = charsReadStatus & 0x80000000;
        numCharsRead = charsReadStatus & ~0xC0000000;
        if (gotJunk) {
