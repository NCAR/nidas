...
StatisticsCruncher::StatisticsCruncher(const SampleTag* stag,statisticsType stype,bool npts):
	numpoints(npts),
	crossTerms(false),
	resampler(0),
	statsType(stype),
	...
	throw(atdUtil::IOException)
{
    if (sampleTags.size() > 0)
    	throw atdUtil::IOException(input->getName(),"StatisticsCruncher",
...
bool StatisticsCruncher::receive(const Sample* s) throw()
{
	...
	if (GET_DSM_ID(id) == 1 && GET_SHORT_ID(id) == 32768) {
	    cerr << atdUtil::UTime(tt).format(true,"%Y %m %d %H:%M:%S.%6f ") <<
		    '(' << GET_DSM_ID(id) << ',' << GET_SHORT_ID(id) <<
		") " << nSamples[0] << ' ';
	    for (i = 0; i < nvarsin; i++)
		cerr << inData[i] << ' ';
...
void StatisticsCruncher::computeStats()
{
    ...
    cerr << "Covariance Sample: " <<
    	atdUtil::UTime(tout-periodUsecs*.5).format(true,"%H:%M:%S");
