...
NetcdfRPCChannel::NetcdfRPCChannel(): fileLength(SECS_PER_DAY),clnt(0),ntry(0)
{
       ...
       throw(atdUtil::IOException)
{
    ...
    if (!stag) throw atdUtil::IOException(getName(),"connect","no samples");
    ...
    clnt = clnt_create(getServer().c_str(), NETCDFSERVERPROG, NETCDFSERVERVERS,"tcp");
    if (clnt == (CLIENT *) NULL)
        throw atdUtil::IOException(getName(),"clnt_create",
    ...
    if ((clnt_stat = clnt_call(clnt, OPENCONNECTION,
	  ...
	  rpcOtherTimeout)) != RPC_SUCCESS) {
        atdUtil::IOException e(getName(),"open",
...
void NetcdfRPCChannel::write(datarec_float *rec) throw(atdUtil::IOException)
{
	...
	throw atdUtil::IOException(getName(),"write",clnt_sperror(clnt,""));
}

void NetcdfRPCChannel::nonBatchWrite(datarec_float *rec) throw(atdUtil::IOException)
{
    int* result = 0;
    enum clnt_stat clnt_stat;
	...
	if (clnt_stat != RPC_SUCCESS) {
		...
		throw atdUtil::IOException(getName(),"write",
			clnt_sperror(clnt,""));
	}
	else {
	    if (ntry > NTRY / 2)
	    	atdUtil::Logger::getInstance()->log(LOG_WARNING,"%s: OK",
		    getName().c_str());
	    break;
	}
    }
...
void NetcdfRPCChannel::flush() throw(atdUtil::IOException)
{
    enum clnt_stat clnt_stat;
    bool serious = false;
    ...
    if (serious)
	throw atdUtil::IOException(getName(),"flush",clnt_sperror(clnt,""));
}

void NetcdfRPCChannel::close() throw(atdUtil::IOException)
{
   ...
   if (clnt) {
	...
	if ((clnt_stat = clnt_call(clnt, CLOSECONNECTION,
	    ...
	    (xdrproc_t) xdr_int, (caddr_t) &result,
	    rpcOtherTimeout)) != RPC_SUCCESS) {
	  atdUtil::IOException e(getName(),"close",clnt_sperror(clnt,""));
	  clnt_destroy(clnt);
	  clnt = 0;
	  throw e;
	}
...
void NetcdfRPCChannel::fromDOMElement(const DOMElement* node)
...
DOMElement* NetcdfRPCChannel::toDOMParent(
    DOMElement* parent)
    throw(DOMException)
{
    DOMElement* elem =
...
DOMElement* NetcdfRPCChannel::toDOMElement(DOMElement* node)
    throw(DOMException)
{
    return node;
}
	...
	throw(atdUtil::IOException)
{
	...
	if (weightsIndex != nvars - 1)
	    throw atdUtil::IOException(conn->getName(),"connect",
	...
	atdUtil::Logger::getInstance()->log(LOG_WARNING,"nc_server OK");
    ...
    if (clnt_stat != RPC_SUCCESS)
	throw atdUtil::IOException(conn->getName(),"define data rec",
...
// #define DEBUG
#ifdef DEBUG
    atdUtil::UTime ut(fsamp->getTimeTag());
