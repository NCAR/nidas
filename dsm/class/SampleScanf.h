...
#ifndef DSM_SERIALPORTSCANF_H
#define DSM_SERIALPORTSCANF_H

/*
 * flex reads SampleScanf.ll and creates SampleScanf.cc.
 * In SampleScanf.cc FLEX_SCANNER is defined, and it also defines
 * yyFlexlexer as below, and includes <FlexLexer.h>.  We
 * put the #ifndef FLEX_SCANNER here to prevent SampleScanf.cc
...
/**
 * Class providing sscanf functionality for parsing ASCII samples.
 * One sets the sscanf format with setFormat then connects the
 * SampleScanf to a SampleSource that is generating
 * samples of ASCII characters.  Each received sample is
 * sscanf'd, the results converted to float (if they aren't
 * already), and the FloatSample distributed to SampleClients.
 * This class does a lexical parse of the sscanf conversion
 * format string to determine the number and types of the
 * conversions.
 */
class SampleScanf: public ScanfFlexLexer, public dsm::SampleClient,
	public dsm::SampleSource {
public:
