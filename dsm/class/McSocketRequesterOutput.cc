...
void McSocketRequesterOutput::offer(atdUtil::Socket* sock)
	throw(atdUtil::IOException)
{
    socket = sock;
...
size_t McSocketRequesterOutput::getBufferSize() const
{
    if (socket) return socket->getReceiveBufferSize();
    else return 16384;
}

Output* McSocketRequesterOutput::clone() const
{
    return new McSocketRequesterOutput(*this);
}
...
void McSocketRequesterOutput::fromDOMElement(const xercesc::DOMElement* node)
...
DOMElement* McSocketRequesterOutput::toDOMParent(
    DOMElement* parent)
    throw(DOMException)
{
    DOMElement* elem =
...
DOMElement* McSocketRequesterOutput::toDOMElement(DOMElement* node)
    throw(DOMException)
{
    return node;
}
