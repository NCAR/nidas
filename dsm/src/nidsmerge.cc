...
private:
    ...
    atdUtil::UTime startTime;
 
    atdUtil::UTime endTime;
...
int NidsMerge::parseRunstring(int argc, char** argv) throw()
{
	...
	case 'e':
	    try {
		endTime = atdUtil::UTime::parse(true,optarg);
	    }
	    catch (const atdUtil::ParseException& pe) {
	        cerr << pe.what() << endl;
		return usage(argv[0]);
	    }
	    break;
	...
	case 's':
	    try {
		startTime = atdUtil::UTime::parse(true,optarg);
	    }
	    catch (const atdUtil::ParseException& pe) {
	        cerr << pe.what() << endl;
		return usage(argv[0]);
...
int NidsMerge::run() throw()
{
    ...
    try {
	FileSet* outSet = new dsm::FileSet();
	...
	for (dsm_time_t tcur = startTime.toUsecs(); tcur <= endTime.toUsecs();
	    ...
	    cout << atdUtil::UTime(tcur).format(true,"%Y %b %d %H:%M:%S");
	    for (unsigned int ii = 0; ii < inputs.size(); ii++)
