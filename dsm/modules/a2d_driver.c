...
{	
	...
	{	
		...
		A2DError(A2DInit(i, &a2d->filter[i][0]));
...
void A2DSetCtr(A2D *a2d)
{
	...
	for(i = 0; i < 8; i++)
	{
		if(a2d->Hz[i] > 0)
		{
			a2d->ctr[i] = (US)((100/a2d->Hz[i]) +0.5);	// Set counter for channel
		}
		else
		{
			a2d->ctr[i] = -1;
...
// The gain can actually go down to .7 before the system has saturation problems
// Check
...
UC A2DSetGain(int A2DSel, int A2DGain)
{
	...
	if(A2DGain < 10 || A2DGain > 25)return(ERRA2DGAIN);
