...
If user id is numeric and not in /etc/passwd inside the container, you'll see a prompt of \"I have no name\", which isn't generally fatal. If it is a string and not in /etc/passwd, podman will fail.
...
# If the image is not already loaded, podman run should(?) pull the
...
If $user isn't a valid user on this podman host, writing files to
...
If $user isn't found in /etc/passwd in the container, podman will fail
...
set -x
exec podman run --rm --user $user:$group --userns=keep-id \
